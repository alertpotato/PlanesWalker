...
using Planeswalker.GameObjects.UnitCard;
using Planeswalker.GameObjects.ArmyField;
...
namespace Planeswalker
{
    ...
    public class Event1 : MonoBehaviour
    {
        ...
        private void CardChosen(GameObject _chosenCard)
        {
            Debug.Log("StartCardChosen in event1");

            if (state == eventState.presentArmies)
            {
        ...
        {
            Debug.Log("StartCellChosen in event1");
        ...
        private void PresentArmies(Hero _yourHero, Hero _enemyHero)
        {
            for (int i = 0; i < _yourHero.bannersList.Count; i++)
            {
                ...
                _cards[i].GetComponent<UnitCardMain>().SetUnitParameters(_yourHero.bannersList[i].unitname, _yourHero.bannersList[i], a);
            }
            for (int i = 0; i < _enemyHero.bannersList.Count; i++)
            {
                ...
                _cardsEnemy[i].GetComponent<UnitCardMain>().SetUnitParameters(_enemyHero.bannersList[i].unitname, _enemyHero.bannersList[i], a);
            }
        }
        private void ChosingArmyEvent() { CreateCardChoice(6); }
        private void StaticChosingArmyEvent() { CreateCardChoice(5); }
        private void CreateCardChoice(int cardsNum)
        {
            for (int i = 0; i < cardsNum; i++)
            {
                var a = Camera.main.ScreenToWorldPoint(new Vector3((Screen.width / (cardsNum + 1)) * (i + 1), Screen.height / 2, 8));
                ...
                _cards.Add(Instantiate(Resources.Load<GameObject>("Prefab/UnitCardMain")));
                _cards[i].GetComponent<UnitCardMain>().SetUnitParameters(newSquad.unitname, newSquad, a);
            }
        }
